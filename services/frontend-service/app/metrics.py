...
frontend_search_queries_total = Counter(
    "frontend_search_queries_total",
    "Total search queries submitted",
    ["authenticated"]
)
...
def track_proxy_request(service: str, endpoint: str, status_code: int, duration: float):
    ...
    frontend_proxy_requests_total.labels(service=service, endpoint=endpoint, status=status_code).inc()
    frontend_proxy_request_duration_seconds.labels(service=service, endpoint=endpoint).observe(duration)
