...
async def run_all_tests():
    """Run all tests."""
    print(f"\n{Colors.BOLD}{'='*60}")
    print(f"  Watchlist Service Test Suite")
    print(f"{'='*60}{Colors.RESET}\n")
    
    print_info(f"Service URL: {BASE_URL}")
    print_info(f"Database: Supabase PostgreSQL")
    ...
    print(f"\n{Colors.BOLD}{'='*60}")
    print(f"  Total: {total} | Passed: {Colors.GREEN}{passed}{Colors.RESET} | Failed: {Colors.RED}{failed}{Colors.RESET}")
    print(f"{'='*60}{Colors.RESET}\n")
    
    if failed == 0:
        print(f"{Colors.GREEN}{Colors.BOLD}SUCCESS: All tests passed!{Colors.RESET}\n")
    else:
        print(f"{Colors.RED}{Colors.BOLD}FAILURE: Some tests failed. Check the logs above.{Colors.RESET}\n")
